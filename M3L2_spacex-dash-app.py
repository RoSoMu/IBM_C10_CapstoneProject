...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df, values = 'count', names = 'class', title = f'Total Success Launches for "{entered_site}"')
        return fig
# TASK 4:
...
def get_scatter(entered_site, payload_range):
    range_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= payload_range[0]) & (spacex_df['Payload Mass (kg)'] <= payload_range[1])]
    if entered_site == 'ALL':
        ...
        fig = px.scatter(filtered_df,
            ...
            y="class",
            color="Booster Version Category",
            title= f'Correlation Between Payload and Mission Outcomes for "{entered_site}"')
        return fig
